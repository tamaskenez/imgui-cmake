...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    io.DisplaySize = MyVec2((float)display_w, (float)display_h);
    ...
    if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED))
    {
    	...
    	io.MousePos = MyVec2((float)mouse_x, (float)mouse_y);   // Mouse position, in pixels (set to -1,-1 if no mouse / on another screen, etc.)
    }
    else
    {
    	io.MousePos = MyVec2(-1,-1);
    }
